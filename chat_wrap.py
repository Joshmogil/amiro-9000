...
def respond(message, feeling):
  ...
  response = openai.Completion.create(
  model="text-davinci-003",
  prompt=f"Respond how a {feeling} person would respond to the message: {message}",
  temperature=1,
  max_tokens=64,
  top_p=1,
  frequency_penalty=0.32,
...
for feeling in ["happy","insightful", "angry", "sad", "stupid"]:
  print(f"ChatGPT responding as a {feeling} person, using prompt wraps:")
  resp = respond("Nonsense", feeling)
